...
// ─────────────────────────────────────────────────────────────────────────────
// IngestionHierarchyBox
// Explains the three-tier Smart Ingestion logic inline below Step 3b.
// Pure presentational — no state, no props. Uses the existing neutral callout
// pattern from this file (border-zinc-200 bg-zinc-50).
// ─────────────────────────────────────────────────────────────────────────────
...
function IngestionHierarchyBox() {
  return (
    <motion.div
    ...
    >
      ...
      <div className="px-3.5 pt-3 pb-1">
        <p className="text-xs text-zinc-500 leading-relaxed">
          The CLI uses a three-tier decision tree to determine which FDA regulatory
          requirement each test maps to. Pick the tier that fits your workflow.
        </p>
      </div>

      {/* Tier rows */}
      <div className="space-y-0 px-3.5 pb-3.5 pt-2.5">
          ...
          return (
            <div
              ...
              className={`border-l-2 pl-3 ${tier.borderColor} ${
                idx < INGESTION_TIERS.length - 1 ? "mb-3" : ""
              }`}
            >
              {/* Row header */}
              <div className="mb-1 flex items-center gap-2">
                <Icon className={`h-3.5 w-3.5 shrink-0 ${tier.iconColor}`} strokeWidth={1.75} />
                <span className="text-xs font-semibold text-zinc-800">
                  {tier.label}
                </span>
                <span
                  className={`inline-flex items-center rounded border px-1.5 py-0.5 font-mono text-[10px] font-medium ${tier.badgeColor}`}
