...
//
// Design: Split-screen enterprise layout, fully light-mode locked.
...
// branding panel is hardcoded to bg-slate-50 / text-slate-900 to match the
// rest of the enterprise palette.
  ...
  return (
    <div className="hidden lg:flex lg:w-[52%] xl:w-[55%] flex-col items-center justify-center bg-slate-50 text-slate-900 px-12 py-12 border-l border-slate-200">
      ...
      <Link href="/" className="mb-auto flex items-center gap-3 group self-start">
        <div className="flex h-9 w-9 items-center justify-center rounded bg-zinc-100 ring-1 ring-zinc-200 group-hover:ring-zinc-400 transition-all duration-200">
      ...
      <div className="flex flex-col items-center text-center max-w-sm py-10">
        <div className="mb-4 inline-flex items-center gap-2 rounded border border-slate-200 bg-slate-100 px-3 py-1">
          <span className="h-1.5 w-1.5 rounded-sm bg-slate-500" />
          <span className="text-xs font-medium text-slate-600">Compliance Vault — Active</span>
        </div>
        <h2 className="text-3xl font-medium leading-tight tracking-tight text-slate-900 xl:text-4xl">
          Your Regulatory
          <br />
          <span className="text-slate-700">Evidence — Secured.</span>
        </h2>
          ...
          {trustPoints.map((point) => {
            ...
            return (
              <div key={point.title} className="flex items-start gap-3">
                <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded border border-slate-200 bg-white">
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center bg-white px-8 py-16 lg:w-[48%] xl:w-[45%]">
      ...
      <Link href="/" className="mb-8 flex flex-col items-center gap-3 lg:hidden group">
        <div className="flex h-12 w-12 items-center justify-center rounded border border-zinc-200 bg-zinc-50">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              disabled={loading}
              className="w-full bg-white border border-gray-300 text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-slate-500 focus:border-slate-500 rounded px-4 py-2 text-sm outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            />
          </div>
  ...
  return (
    <div className="relative flex min-h-screen flex-col lg:flex-row">
      ...
      <Link
        ...
        className="absolute left-4 top-4 z-20 inline-flex items-center gap-2 rounded border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 px-4 py-2.5 text-sm font-medium transition-all"
      >
